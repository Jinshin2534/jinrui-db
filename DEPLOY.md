...
## 0. 最短: Render + 同梱シード(外部DB不要・推奨)

リポジトリに 613名分の `data/*.hrfl` を同梱済み。起動時に `tools/boot.mjs` が空のPGliteを同梱.hrflから再構築して配信するので、**マネージドDBは不要**。

1. <https://render.com> に GitHub でログイン。
2. **New +** → **Blueprint** → リポジトリ `Jinshin2534/jinrui-db` を選択(`render.yaml` を自動検出)。
   - 手動なら **New + → Web Service**: Root Directory=`tools` / Build=`npm install --omit=dev` / Start=`node boot.mjs`。
3. **Apply / Create**。ビルド後、起動時に約9秒でシード → 公開。
4. 払い出されたURL(`https://jinrui-db-xxxx.onrender.com`)が公開URL。

> 無料プランは無操作でスリープし、次アクセス時にコールド起動(数十秒＋シード約9秒)。
> データ更新は `data/*.hrfl` を差し替えて push すれば、再デプロイ時に反映。

以下は **永続/大規模運用でマネージドPostgresに載せる**場合の別案。
