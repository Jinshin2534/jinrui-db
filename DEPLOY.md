...
> **メモリ注意**: PGlite(WASM版Postgres)で613名を配信すると約650MB使う。Render等の**無料枠(512MB)には収まらない**。
> 同梱PGliteだけで動かすのは **RAMが1GB以上のホスト**に限られる。無料枠で公開するなら、以下の **Neon(マネージドPostgres)** を使う(サーバは約68MBに収まる)。
...
1行ずつのロード(`db.mjs load`)は高遅延の遠隔DBだと遅すぎる(例: 10件で約60秒)。
**ローカルPGliteを高速に作ってから、`seed-remote.mjs` で一括転送**する(55,000行が数秒)。

```bash
cd 人類DB
# 1) ローカルPGliteを高速に構築(DATABASE_URLは付けない)
node tools/db.mjs init
node tools/db.mjs load format/examples data/*.hrfl     # 613名(約9秒)
# 2) Neonへ schema適用＋全行をバルク転送(数秒)
DATABASE_URL="$DATABASE_URL" node tools/seed-remote.mjs
# 確認
DATABASE_URL="$DATABASE_URL" node tools/db.mjs query "select count(*) from person p where exists(select 1 from name where person_id=p.id)"
```
